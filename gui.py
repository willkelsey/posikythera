...
search_event_date = Label(root, text="Search for event by date range").grid(row=7)
...
#trying to get hohmann to display on screen
# hohmann_text = Text(root, height=5, width=30).grid(row=5, column=3)
# hohmann_text.insert(END, "Setup for hohmann text")
# buttons
...
button_run_sim = Button(root, text="Press to run simulation", command=run_sim).grid(row=11)

# event_scrollbar = Scrollbar(root).pack(side=RIGHT, fill=Y)
# event_list = Listbox(root, yscrollcommand=event_scrollbar.set)
# for line in range(100):
#     event_list.insert(END, 'This is line number' + str(line))
# event_list.pack(side=LEFT, fill=BOTH)
# event_scrollbar.config(command=event_list.yview)

#keep gui open until closed
mainloop()
